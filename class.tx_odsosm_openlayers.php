	...
	public function getMarker($item,$table){
		...
		switch($table){
			...
			case 'tt_address':
				...
				$jsMarker.="mapMarker(".$this->config['id'].",layerMarkers_".array_search($item['group_title'],$this->group_titles).",".$item['tx_odsosm_lat'].",".$item['tx_odsosm_lon'].",'".$icon."',".$marker['size_x'].",".$marker['size_y'].",".$marker['offset_x'].",".$marker['offset_y'].",'".strtr($item['popup'],$this->escape_js)."',".intval($this->config['show_popups']).");\n";
			break;
